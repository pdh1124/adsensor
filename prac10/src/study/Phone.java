...
class PhoneInfo implements Serializable {
	// 직렬화를 위해서는 Serializable(시리얼라이저블) 인터페이스를 구현하라.
	// 직렬화는 객체를 바이너리값으로 변경해서 원격지로 보내는 기술.
	...
	@Override //hashCode()를 부모클래스(String.class)에서 가져와 수정함
	public int hashCode() { 
		return name.hashCode(); //hashCode() : 해쉬코드를 가져온다. ex)44502158
		//뽑는 이유는 두 객체(같은 내용은 equals를 씀)가 같은 객체인지 확인하기 위해서.
	}

	@Override //equals()를 부모클래스(String.class)에서 가져와 수정함
	public boolean equals(Object obj) {
		//매개변수로 Object(String이든 int든 PhoneInfo객체든 다 가져올 수 있음)를 가져온다
		if (obj instanceof PhoneInfo) {
			PhoneInfo n = (PhoneInfo) obj;
			if (n.name.equals(name)) { //equals(a)는 a의 내용과 앞에 내용을 비교해서 맞으면 true 
				return true;
			}
		}
		return false;
...
class PhoneBookManager {
	...
	*/
	...
	// 입력 기능
	void readData() {
		ObjectInputStream objInputStream = null; //오브젝트 객체 생성
		FileInputStream inputStream = null; //파일
		
		
		//파일이 없다면 파일을 만들어라
//		if(!file1.isFile()) { //isFile()은 경로인지 아닌지 확인
//			try {
...
//		objInputStream.writeObject(people);
	}
